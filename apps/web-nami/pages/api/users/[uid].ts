...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    return res.status(200).json({
      uid: uid,
      user_id: userData.user_id, // PersonId from database
      personId: userData.user_id, // PersonId in camelCase for consistency
      email: userData.email,
      username: userData.username,
      full_name: userData.full_name,
      name: userData.full_name, // Also include as 'name' for compatibility
